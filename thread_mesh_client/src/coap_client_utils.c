...
#define RESPONSE_POLL_PERIOD 1000
...
coap_reply_t reply_of_led(const struct coap_packet *response,
			    struct coap_reply *reply,
			    const struct sockaddr *from)
{
	LOG_INF("########################## REPONSE RECEIVED ###################");
...
static void toggle_mesh_lights(struct k_work *item)
{
	...
	coap_send_request(COAP_METHOD_PUT,
			  ...
			  light_option, &command, sizeof(command), reply_of_led);

	LOG_INF("###############SenT multicast mesh 'light' request###################");
}
